...
public class controldownloadpdf extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Documento> documentoList = dDocumento.SelectByIdDocumento(doc);
        if (!documentoList.isEmpty()) {
            Documento dataDocumento = documentoList.get(0);
            byte[] archivopdf = dataDocumento.getArchivoOrigen();
            String filename = dataDocumento.getNombreDocumento();

            // Configurar la respuesta HTTP
            response.setContentType("application/pdf");
            response.setHeader("Content-Disposition", "attachment; filename=" + filename + "_" + IdDocumento + ".pdf");
